...
#
# Collecting Data from IMU for covariences
#
...
def listener():
    ...
    global linearCSV
    global angularCSV
    global orientationCSV
    global linearCSVFile
    global angularCSVFile
    global orientationCSVFile
    ...
    linearCSVFile = open('imu_sample_linear.csv', 'w')
    angularCSVFile = open('imu_sample_angular.csv', 'w')
    orientationCSVFile = open('imu_sample_quaternion.csv', 'w')
    linearCSV = csv.writer(linearCSVFile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    angularCSV = csv.writer(angularCSVFile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    orientationCSV = csv.writer(orientationCSVFile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
...
def dispose():
    ...
    if None != imu_pub:
        imu_pub.unregister()
        imu_pub = None
    if None != linearCSVFile:
        linearCSVFile.close()
        linearCSVFile = None
        linearCSV = None
    if None != angularCSVFile:
        angularCSVFile.close()
        angularCSVFile = None
        angularCSV = None
    if None != orientationCSVFile:
        orientationCSVFile.close()
        orientationCSVFile = None
        orientationCSV = None
...
def on_imu(qx,qy,qz,qw, gx,gy,gz, ax,ay,az):
    ...
    orientationCSV.writerow([qx,qy,qz,qw])
    angularCSV.writerow([gx,gy,gz])
    linearCSV.writerow([ax,ay,az])
